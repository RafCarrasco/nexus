  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <ChevronRight className="h-4 w-4 text-zinc-400 group-hover:text-violet-600 shrink-0" />
      </div>
      <div className="mt-4 grid grid-cols-3 gap-2 text-center">
        ...
        <div className="rounded-lg bg-zinc-50 py-2">
          ...
          <div className={`text-sm font-semibold ${openIncidents > 0 ? 'text-red-600' : 'text-zinc-900'}`}>
            {openIncidents > 0 && <AlertTriangle className="inline h-3 w-3 -mt-0.5 mr-1" />}
            {openIncidents}
          </div>
        </div>
        <div className="rounded-lg bg-zinc-50 py-2">
