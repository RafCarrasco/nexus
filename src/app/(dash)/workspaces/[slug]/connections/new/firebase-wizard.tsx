...
export function FirebaseWizard({ workspaceSlug, workspaceId, workspaceName }: Props) {
  ...
  function StepIcon({ n }: { n: Step }) {
    ...
    return (
      <div className="h-6 w-6 rounded-full bg-zinc-100 text-zinc-400 flex items-center justify-center shrink-0">
        ...
        {steps.map((s) => (
          <button
            ...
            className={`w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm transition-colors ${
              step === s.n
                ? 'bg-violet-50 text-zinc-900 font-medium'
                : step > s.n
      ...
      <div className="space-y-6">
        ...
        {step === 1 && (
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm space-y-5">
            ...
            <ol className="space-y-3 text-sm text-zinc-700 list-none">
              {[
                <>
                ...
                <>
                  ...
                  <span className="font-mono bg-zinc-100 text-xs px-1.5 py-0.5 rounded">⚙️</span>{' '}
                ...
                <>
                  ...
                  <code className="bg-zinc-100 text-xs px-1.5 py-0.5 rounded font-mono">
                    .json
                  </code>
                </>,
              ].map((item, i) => (
                <li key={i} className="flex gap-3 items-start">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-violet-100 text-violet-700 text-xs font-bold mt-0.5">
                    {i + 1}
                  </span>
                  <span>{item}</span>
                </li>
        ...
        {step === 3 && sa && (
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm space-y-5">
            ...
            <dl className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 space-y-4 text-sm">
              <div>
                <dt className="text-xs uppercase tracking-wider text-zinc-500 mb-0.5">
                  Project ID
                </dt>
                <dd className="font-mono text-zinc-900 font-medium">{sa.project_id}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wider text-zinc-500 mb-0.5">
                  Service account email
                </dt>
                <dd className="font-mono text-zinc-900 break-all">{sa.client_email}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wider text-zinc-500 mb-0.5">
                  Aplicativo Nexus
                </dt>
                <dd className="text-zinc-900">{workspaceName}</dd>
              </div>
            </dl>
        ...
        {step === 4 && sa && (
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm space-y-5">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-zinc-500">Como vai aparecer na lista de conexões.</p>
            </div>
            ...
            <details className="rounded-xl border border-zinc-200 bg-zinc-50 group">
              <summary className="cursor-pointer text-sm font-medium text-zinc-700 px-4 py-3 select-none list-none flex items-center justify-between">
  ...
  --role="roles/monitoring.viewer"`}
        ...
        {step === 5 && sa && (
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm space-y-5">
            ...
            <dl className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 space-y-3 text-sm">
              <div className="flex justify-between gap-4">
                <dt className="text-zinc-500 shrink-0">Nome</dt>
                <dd className="text-zinc-900 font-medium text-right">{name}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-zinc-500 shrink-0">Projeto Firebase</dt>
                <dd className="text-zinc-900 font-mono text-right">{sa.project_id}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-zinc-500 shrink-0">Service account</dt>
                <dd className="text-zinc-900 font-mono text-right break-all text-xs">
                  {sa.client_email}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-zinc-500 shrink-0">Aplicativo</dt>
                <dd className="text-zinc-900 text-right">{workspaceName}</dd>
              </div>
              {billingId && (
                <div className="flex justify-between gap-4">
            ...
            {syncStatus && (
              <div className="flex items-center gap-2 rounded-md bg-violet-50 border border-violet-200 p-3 text-sm text-violet-900">
