...
export default async function WorkspacesPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Aplicativos"
        action={<Button asChild><Link href={'/workspaces/new' as never}>Novo aplicativo</Link></Button>}
      />
      {workspaces.length === 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {workspaces.map((w) => {
          ...
          const cost = resources.reduce((s, r) => s + r.costSnapshots.reduce((ss, x) => ss + Number(x.amount), 0), 0);
